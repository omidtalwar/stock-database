...
$anyPurchases = array_sum(array_column($purchasesByCur, 'cnt')) > 0;
$anyPays      = array_sum(array_column($paysByCur, 'cnt')) > 0;
$rateUSD = $rates['USD']; $ratePKR = $rates['PKR'];
?>
...
<div class="mb-1 text-muted small fw-semibold" style="letter-spacing:.3px;text-transform:uppercase;font-size:0.65rem;">
    <i class="bi bi-arrow-down-circle me-1"></i>Purchases — by currency
</div>
<div class="row g-2 mb-4">
    <?php foreach ($curDefs as $cur => $def):
        $d = $purchasesByCur[$cur];
        if ($d['cnt'] === 0) continue;
    ?>
    ...
    <div class="col-6 col-md-3">
        <div class="card h-100" style="background:rgba(0,0,0,0.02);">
            <div class="card-body py-3 text-center">
                ...
                <div class="fw-bold" style="font-size:1.05rem;color:#333;"><?= formatAFN($totalPurchasedAfn) ?></div>
                <div class="text-muted" style="font-size:0.72rem;">≈ <?= formatMoney(fromAFN($totalPurchasedAfn,$rateUSD),'USD') ?></div>
                <div class="text-muted" style="font-size:0.72rem;">≈ <?= formatMoney(fromAFN($totalPurchasedAfn,$ratePKR),'PKR') ?></div>
            </div>
        </div>
    </div>
</div>
...
<?php if ($anyPays): ?>
<div class="mb-1 text-muted small fw-semibold" style="letter-spacing:.3px;text-transform:uppercase;font-size:0.65rem;">
    <i class="bi bi-cash-coin me-1"></i>Payments Made — by currency
</div>
<div class="row g-2 mb-4">
    <?php foreach ($curDefs as $cur => $def):
        $d = $paysByCur[$cur];
    ...
    <div class="col-6 col-md-3">
        <div class="card h-100" style="background:rgba(16,124,16,0.04);border-color:rgba(16,124,16,0.20);">
            <div class="card-body py-3 text-center">
                ...
                <div class="fw-bold text-success" style="font-size:1.05rem;"><?= formatAFN($totalPaidAfn) ?></div>
                <div class="text-muted" style="font-size:0.72rem;">≈ <?= formatMoney(fromAFN($totalPaidAfn,$rateUSD),'USD') ?></div>
                <div class="text-muted" style="font-size:0.72rem;">≈ <?= formatMoney(fromAFN($totalPaidAfn,$ratePKR),'PKR') ?></div>
            </div>
        </div>
    </div>
</div>
